...
static DWORD s_dwExplorerProcessID = 0; //父进程id  // windows资源管理器进程id
...
static bool EnumProcessFunc(IN  void *callBackParameter, IN PROCESSENTRY32* pProcessEntry)
{
	...
	{
		// 正常通过鼠标点击启动的可执行软件，父进程都是explorer资源管理器。
		// 这里判断是否是资源管理器，不是说明，启动方式有问题。
		if (pProcessEntry->th32ParentProcessID != s_dwExplorerProcessID) // 调用该DLL的进程的父进程不是资源管理器进程
		{
...
DWORD WINAPI KillHandleThread(LPVOID lparam)
{
	while (1)
	{
		...
		Loop:
			PVOID pBuffer = NULL;
			pBuffer = VirtualAlloc(NULL, nSize, MEM_COMMIT, PAGE_READWRITE);//如果在内核形态这里是使用Pool或者其他内核内存分配形态！！！
			if (pBuffer)
			{
